...
for i in xrange(0,maxsteps):
	...
	s=(np.random.random_sample())*step(i)
	###check which term of fisher info is more away of optimum
	fish1=(a[p,q+1]-a[p,q])*(b[p+1,q]-b[p,q])/(dx*dx)-I
	fish2=(a[p+1,q]-a[p,q])*(b[p,q+1]-b[p,q])/(dx*dx)
	###if first term is more off - do random step
	if abs(fish1)>abs(fish2):
		...
		if mu==0:
			...
			if di==0:
				...
				test[p,q]=test[p,q]-s
	####if second one ist more off - do step to make second closer to zero
	else:
		noise1=(np.random.random_sample()-0.5)*noise
		noise2=(np.random.random_sample()-0.5)*noise
		noise3=(np.random.random_sample()-0.5)*noise
		noise4=(np.random.random_sample()-0.5)*noise


	###calc grad_y of field a and grad_x of field b, pick the larger one (both should be zero if optimal)
		grada=abs(a[p,q]-a[p-1,q])+abs(a[p+1,q]-a[p,q])
		gradb=abs(b[p,q]-b[p,q-1])+abs(b[p,q+1]-b[p,q])

		if (grada+noise1)>(gradb+noise2):
			mu=0
			test1=np.copy(a)
			test2=np.copy(a)
			####make step in both directions and pick that one which decreased the grad most
			test1[p,q]=test1[p,q]+s
			test2[p,q]=test2[p,q]-s

			gradanew1=abs(test1[p,q]-test1[p-1,q])+abs(test1[p+1,q]-test1[p,q])
			gradanew2=abs(test2[p,q]-test2[p-1,q])+abs(test2[p+1,q]-test2[p,q])

			if (gradanew1+noise3)<(gradanew2+noise4):
		...
		elif (grada+noise1)<(gradb+noise2):
			mu=1
			test1=np.copy(b)
			test2=np.copy(b)
			...
			gradbnew1=abs(test1[p,q]-test1[p,q-1]+noise3)+abs(test1[p,q+1]-test1[p,q])
			gradbnew2=abs(test2[p,q]-test2[p,q-1]+noise4)+abs(test2[p,q+1]-test2[p,q])

			if (gradbnew1+noise3)<(gradbnew2+noise4):
	...
	if delta>0:  			##got better
		lastgoodone=0
		error=newerror
		print 'gooooooooodddddddddddddddddddddd'
		goodstep=goodstep+1	
		if mu==0:		##do step
			a=test
		elif mu==1:
