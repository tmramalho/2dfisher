...
################################################################################
#################input
N = 40 ###total size of variable grid without boundaries
M = 40
dx=1/float(N-1)
dy=1/float(M-1)
T = M*N
...
################optimization for validation of mat
#f=np.append(f0a,f0b)
...
for n in xrange(0,maxiter):
	...
	areal=smaller(ftota)
	breal=smaller(ftotb)
	f=np.append(areal,breal)
	if (testscore(f)<100000) and mem==0:
		plot(f)
		mem=1
	if (testscore(f)<10000) and mem1==0:
		plot(f)
		mem1=1
	if (testscore(f)<1000) and mem2==0:
		plot(f)
		mem2=1
	if (testscore(f)<100) and mem3==0:
		plot(f)
		mem3=1
fa=smaller(ftota)
fb=smaller(ftotb)
